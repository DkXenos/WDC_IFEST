...
function ChatsPageContent() {
  ...
  return (
    <main className={`fixed inset-0 flex z-0 overflow-hidden pt-[72px] pb-[88px] ${isVideoOn ? 'bg-black' : (isDarkTheme ? 'bg-[oklch(0.145_0_0)]' : 'bg-[#E0C9B6]')} ${!isBlurOn ? 'disable-chat-blur' : ''} transition-colors duration-500`}>
      ...
      <div className="flex flex-col w-full h-full max-w-[1400px] mx-auto p-4 lg:p-6 pb-2 gap-4 relative z-10 transition-all duration-500">
        ...
        <header className={`flex w-fit mx-auto min-h-[56px] ${containerBg} border ${borderColor} rounded-full shadow-lg px-4 py-2 items-center justify-center gap-3 shrink-0 relative z-50 transition-all duration-300`}>
          
          <button
            onClick={() => setIsVideoOn(!isVideoOn)} 
            className={`w-10 h-10 flex items-center justify-center rounded-full transition-all border shadow-sm ${isVideoOn ? 'bg-emerald-500/80 border-emerald-400 text-white' : `${hoverBg} ${borderColor} ${textColor}`}`}
          ...
          <button
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-full transition-all border shadow-sm ${isBlurOn ? 'bg-emerald-500/80 border-emerald-400 text-white' : `${hoverBg} ${borderColor} ${textColor}`}`}
          ...
          <button 
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-full transition-all border shadow-sm ${!isDarkTheme ? 'bg-emerald-500/80 border-emerald-400 text-white' : `${hoverBg} ${borderColor} ${textColor}`}`} 
