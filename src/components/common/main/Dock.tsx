...
import { FiHome, FiMessageSquare, FiSettings, FiCalendar, FiFolder, FiUser, FiBell, FiLock, FiMonitor, FiMusic } from "react-icons/fi";
...
export default function Dock() {
  ...
  const { 
    ...
    isBlurOn, setIsBlurOn,
    activeVideoId, setActiveVideoId,
    unlockedVideos, setUnlockedVideos,
                        ...
                        <div className={`w-60 shrink-0 border-r ${borderColor} bg-white/20 dark:bg-black/20 flex flex-col overflow-y-auto`}>
                          ...
                          <div className="flex items-center gap-3 mb-6 px-5 mt-2">
                            ...
                            <div className="flex flex-col">
                              <span className={`text-[13px] font-bold ${textColor} leading-tight`}>User Profile</span>
                              <span className={`text-[11px] ${mutedTextColor} mt-0.5`}>Apple ID, iCloud</span>
                            </div>
                          </div>
                          ...
                          <div className="flex flex-col gap-1 px-3 pb-6">
                            {SETTINGS_TABS.map((tab, idx) => (
                              ...
                              >
                                <div className={`w-5 h-5 rounded flex items-center justify-center text-white shadow-sm ${
                                  idx === 0 ? 'bg-blue-500' : idx === 1 ? 'bg-indigo-500' : idx === 2 ? 'bg-gray-500' : idx === 3 ? 'bg-red-500' : 'bg-orange-500'
                                }`}>
                                   <tab.icon size={12} />
                                </div>
                                {tab.label}
                        ...
                        <div className={`flex-1 p-10 overflow-y-auto bg-white/40 dark:bg-black/40`}>
                            ...
                            <div>
                              ...
                              <div className="flex flex-col gap-6">
                                ...
                                <div className="flex flex-col gap-2">
                                  <label className={`font-semibold ${textColor} text-sm`}>Color Scheme</label>
                                  <div className={`flex items-center p-1 rounded-lg border ${borderColor} bg-black/5 w-fit gap-1`}>
                                    <button onClick={() => setIsDarkTheme(false)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${!isDarkTheme ? 'bg-white dark:bg-neutral-800 shadow-sm' : 'bg-transparent hover:bg-black/5'} ${textColor}`}>Light</button>
                                    <button onClick={() => setIsDarkTheme(true)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${isDarkTheme ? 'bg-white dark:bg-neutral-800 shadow-sm' : 'bg-transparent hover:bg-black/5'} ${textColor}`}>Dark</button>
                                  </div>
                                </div>

                                <div className="flex flex-col gap-2 mt-2">
                                   <div className="flex items-center justify-between">
                                     <span className={`text-sm font-semibold ${textColor}`}>Enable Glassmorphism Blur</span>
                                     <div onClick={() => setIsBlurOn(!isBlurOn)} className={`w-10 h-6 ${isBlurOn ? 'bg-emerald-500' : 'bg-black/20 dark:bg-white/20'} rounded-full relative cursor-pointer transition-colors duration-300`}>
                                ...
                                <div className="flex flex-col gap-2 mt-4 pt-6 border-t border-black/10 dark:border-white/10">
                                  <label className={`font-semibold ${textColor} text-sm mb-2`}>Video Background Wallpaper</label>
                                  <div className="flex flex-col gap-2">
                          ...
                          {activeSettingsTab === 'music' && (
                            <div>
                              <div className="flex items-center justify-between mb-6">
                                <h2 className={`text-2xl font-bold ${textColor}`}>Background Music</h2>
                                <button 
                          ...
                          {['account', 'notifications', 'privacy'].includes(activeSettingsTab) && (
                            <div className="flex flex-col items-center justify-center h-full text-center pb-20 opacity-50 pt-10">
                              ...
                              <h3 className={`text-lg font-bold ${textColor}`}>Work in Progress</h3>
                              <p className={`text-sm ${mutedTextColor} mt-2 max-w-[250px]`}>This settings module is currently under active development.</p>
                            </div>
                          )}
                        </div>
                      </div>
