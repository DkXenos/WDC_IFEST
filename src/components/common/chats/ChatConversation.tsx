    ...
    <section className="flex-1 h-full min-w-0 flex flex-col bg-transparent relative z-20">
      ...
      <header className={`h-[84px] shrink-0 flex items-center justify-between px-6 ${panelBg} rounded-[1.5rem] border ${borderColor} shadow-sm mt-4 mr-4 mx-2 transition-colors duration-300`}>
        <div className="flex items-center gap-4 min-w-0">
          <div className="relative shrink-0">
            ...
            {user.online && (
              <div className={`absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-emerald-400 rounded-full border-[3px] ${isDarkTheme ? 'border-[#1a1a1a]' : 'border-white'}`} />
            )}
          </div>
          <div className="flex flex-col min-w-0">
            <h2 className={`text-[16px] font-bold ${textColor} truncate m-0`}>
              {chat.name}
            </h2>
            <p className={`text-[12px] font-medium ${emeraldText} m-0 truncate mt-0.5`}>
      ...
      <div className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-4">
        ...
        <div className="flex justify-center mb-6">
          <span className={`text-[12px] font-bold ${textColor} opacity-80 ${panelBg} px-5 py-1.5 rounded-full shadow-sm border ${borderColor} transition-colors duration-300`}>
            Today
          </span>
        </div>
      ...
      <footer className="shrink-0 p-5 lg:p-6 mb-2">
        <div className={`flex items-center gap-3 ${panelBg} rounded-full p-2 pl-4 shadow-lg border ${borderColor} mx-4 transition-colors duration-300`}>
          
          {/* Emoji */}
          <button className={`shrink-0 w-8 h-8 flex items-center justify-center ${mutedTextColor} hover:${textColor} transition-colors`} aria-label="Emoji">
          ...
          <input
            type="text"
            className={`flex-1 min-w-0 h-10 bg-transparent text-[14px] ${textColor} font-medium placeholder:opacity-50 focus:outline-none`}
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button className={`w-9 h-9 flex items-center justify-center rounded-full ${mutedTextColor} hover:${textColor} ${hoverBg} transition-colors`} aria-label="Camera">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="12" cy="13" r="4" />
                <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" />
              </svg>
            </button>
            <button className={`w-9 h-9 flex items-center justify-center rounded-full ${mutedTextColor} hover:${textColor} ${hoverBg} transition-colors`} aria-label="Gallery">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
